...
- **users** — `id`, `username` (unique, lower-indexed), `displayName`, `passwordHash`, `email` (nullable, lower-unique; required at registration since chunk 7, null for grandfathered accounts — no email = no password reset), `notifyOnDecide` (opt-in decided-round emails, 9d), `createdAt`.
